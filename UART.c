...
extern void Configurar_UART2(uint32_t Baudrate)
{
    ...
    SYSCTL->RCGCGPIO |= (1<<0);  //PÁG 340

    GPIOD->DIR =(1<<1)|(0<<0); //PÁG 653
    GPIOD->AFSEL = (1<<7)|(1<<6); //PÁG 344
    GPIOD->PCTL =(0x01<<28)|(0x01<<24);
    GPIOD->DEN =(1<<7)|(1<<6);

    UART2->CTL = (0<<9)|(0<<8)|(0<<0); //PÁG 918
    ...
    UART2->IBRD = 104; // SALIO 10
    UART2->FBRD = 11; //SALE 54.9444 COLOCAMOS 55
    //PÁG 916 LINEA DE CONTROL
    UART2->LCRH = (0X3<<5) | (0<<4);
    //PARA DESABILITAR LA PILA
    UART2->CC = 0X0;

    UART2->CTL = (1<<0) | (1<<8) | (1<<9); //PÁG 918 SE HABILITA EL UART
